...
class MaterialActivityTest {
    ...
    @Test
    fun guessWrong() {
        val res = activityTestRule.activity.resources
        ...
        for (n in 1..10) {
            if (n != secret) {
                ...
                val message =
                    if (n > secret) res.getString(R.string.smaller)
                    else res.getString(R.string.bigger)
                onView(withText(message)).check(matches(isDisplayed()))
                onView(withText(res.getString(android.R.string.ok))).perform(click())
            }
        }
    }
